...
- Chronic Understocking Report ← *COMPLETE* (see `engine/understocking.py`, migration 029). Answers "what should each store be stocking more of instead of waiting for transfers?" Flags SKUs below reorder_point ≥30% of observed snapshot days (target 90d, adapts to actual history), top 20 per location ranked by `priority = stockout_pct × avg_daily_demand × unit_cost`. Companion display metric `inventory_value_at_risk = avg_daily × unit_cost × 21`. Excludes 6 retired branches + LOC-021 INTERNET + LOC-025 MAIN DC. Suggested min = avg_daily × 21 (7d lead + 14d buffer), where avg_daily comes from precomputed `sku_master.avg_weekly_units / 7`. Same-day reruns are fully idempotent (engine deletes today's rows before insert; post-write integrity assertion confirms persisted count == built count). Wired into `WEEKLY_STAGES` (after dead_stock); dashboard panel "Chronic Understocking by Location" with location dropdown defaulting to highest value-at-risk + CSV export; assistant context section registered. **Architecture note**: first attempted as a single Postgres function called via `.rpc()` — exceeded PostgREST `statement_timeout` (57014) on the production cube. Final design uses paginated supabase-py reads (same pattern as `ml/dead_stock.py` — `_paginate` with retry/reconnect on 502/57014), accumulating per-(sku,loc) counters in Python, then bulk `.in_()` lookups against sku_master for the few hundred chronic SKUs. Heliumdb (DATABASE_URL) is Replit's local empty Postgres, NOT Supabase — direct psycopg2 cannot reach the data; everything must go through PostgREST.
