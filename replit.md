...
- Chronic Understocking Report ← *COMPLETE* (see `engine/understocking.py`, migration 029). Answers "what should each store be stocking more of instead of waiting for transfers?" Server-side aggregation in Postgres function `fn_run_understocking_report(...)` — single `.rpc()` call, no PostgREST pagination. Flags SKUs below reorder_point ≥30% of observed snapshot days (target 90d, adapts to actual history), top 20 per location ranked by `priority = stockout_pct × avg_daily_demand × unit_cost`. Excludes 6 retired branches + LOC-021 INTERNET + LOC-025 MAIN DC. Suggested min = avg_daily × 21 (7d lead + 14d buffer). Same-day reruns are fully idempotent (function does `DELETE FROM understocking_report WHERE report_date = today` before insert; engine does post-write integrity assertion `len(persisted) == rows_built`). Wired into `WEEKLY_STAGES` (after dead_stock); dashboard panel "Chronic Understocking by Location" with location dropdown defaulting to highest value-at-risk + CSV export; assistant context section registered. **DB pattern**: when supabase-py pagination is too slow (multi-million-row joins), put the aggregation in a Postgres function and call via `client.rpc()` — heliumdb (DATABASE_URL) is Replit's local empty Postgres, NOT Supabase, so direct psycopg2 won't work for this data.
